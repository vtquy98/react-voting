    ...
    Selectpicker.prototype = {
      ...
      init: function() {
        ...
        if (
          ...
          this.options.lazyLoadLiElements === true
        ) {
          ...
          console.log(
            'Selectpicker option lazyLoadLiElements=true is incompatible with option width="auto".  Option \'lazyLoadLiElements\' has been reset to false, however this may cause serious performance degradation.'
          );
      ...
      createLi: function() {
        ...
        if (
          !this.options.initInProcess ||
          this.options.lazyLoadLiElements !== true
        ) {
          // skip LI creation when lazy loading
      ...
      render: function(updateLi) {
        ...
        if (
          ...
          // skip this if we are lazy loading
          $selectOptions.each(function(index) {
        ...
        var selectedItems =
          this.multiple || this.options.singleSelectPerfTweak !== true
            ? $selectOptions
                .map(function() {
                  if (this.selected) {
            ...
            : // only single select - do not need to iterate every single option
              [
                createOptionText(
                  this.$element[0][this.$element[0].selectedIndex]
                )
              ];
        ...
        if (
          ...
          // this is very expensive on IE and we haven;t really rendered the select if we are lazy loading, so hold off on this event
